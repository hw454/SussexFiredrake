...
def linear_form_func(varkey,var_cls,testvar):
...
def Simulations(filename,path,MeshFolder,ResultsFolder,varnames,ind=0):
    ...
    *Saves numpy arrays for the variables over discrete space and time.
    ...
    vars_cls, eNorm, tNorm,eDiv,tDiv,tvec,vargs=one_step_loop(vars_cls,pdes_par_dict,tvec,path,MeshFolder,ResultsFolder,Outfiles)
    eta1_cls,the1_cls=vars_cls
    #plotting.plot(eta1)
    ...
    return eta1_cls,the1_cls,parstr,pdes_par_dict,vargs
...
def first_step(vars_cls,consts_dict):
    ...
    v0      = TestFunction(V)
    v1      = TestFunction(V)
    L0=linear_form_func(var0_cls.char_key,var0_cls,v0) # Linear form for eta with time step 1
    L1=linear_form_func(var1_cls.char_key,var1_cls,v1)
    a0=bilinear_form_func(var0_cls.char_key,tmpv0,v0,consts_dict)
    a1=bilinear_form_func(var1_cls.char_key,tmpv1,v1,consts_dict)
...
def update_step(vars_cls,consts_dict):
    ...
    v0      = TestFunction(V)
    v1      = TestFunction(V)
    L0=linear_form_func(var00_cls.char_key,var00_cls,v0) # Linear form for eta with time step 1
    L1=linear_form_func(var10_cls.char_key,var10_cls,v1)
...
def one_step_loop(vars_cls,consts_dict,tvec,path,MeshFolder="",PlotFolder="",Outfiles=()):
    ...
    for tstep,t in enumerate(tvec):
        ...
        Lvec,avec=update_step((var00_cls,var01_cls),
        consts_dict)
        a0,a1=avec
        L0,L1=Lvec
...
def Plot_tstep(var_cls,tstep,path,PlotFoldername,consts_dict):
    ...
    return
def Plotting(var_cls,nt,path,PlotFoldername,parstr,vargs):
    ''' Plot an animation of a tricontour plot for the variable `var_cls`.
    Save plot at path+PlotFoldername+var_cls.name+'.mp4'
    :param var_cls: `SpatialVariable` object with results in.
    :param path: Location for the output
...
def main(path,filename,simon=0,ind=0,simstep=1):
    ...
    for i,var in enumerate(var_clslist):
        Plotting(var,nt,path,ResultsFolder,parstr,varr[i])
